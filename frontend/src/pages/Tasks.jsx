...
export const Tasks = () => {
    ...
    const [guidedTaskForm, setGuidedTaskForm] = useState({
        module_id: '',
        taskType: 'Assignment',
        title: '',
        urgency: 'Medium',
    ...
    const generateTaskTitle = (g) => {
        const moduleCode = modules.find((m) => m.id === g.module_id)?.module_code || 'Task';
        return g.title || `${moduleCode} ${g.taskType}`;
    };

    const getModuleName = (id) => modules.find((m) => m.id === id)?.module_name || 'Unknown Module';

    const getPriorityLabel = (p) =>
        ({ 1: 'Low', 2: 'Low-Medium', 3: 'Medium', 4: 'Medium-High', 5: 'High' })[Number(p)] || 'Medium';
    ...
    const handleGuidedTaskSubmit = async (e) => {
        ...
        const est =
            guidedTaskForm.estimated_minutes === '' || guidedTaskForm.estimated_minutes == null
                ? null
    ...
    const handleCreateOrUpdate = async (e) => {
        ...
        const est =
            formData.estimated_minutes === '' || formData.estimated_minutes == null
                ? null
    ...
    const handleBulkImportSubmit = async (e) => {
        ...
        const fallback = modules.length === 1 ? modules[0].id : null;
        const created = [];
        const skipped = [];

        for (const line of lines) {
            try {
                const moduleMatch = modules.find((m) =>
                    line.toUpperCase().includes(m.module_code.toUpperCase())
                );

                const moduleId = moduleMatch?.id || fallback;
                ...
                const title =
                    line.replace(new RegExp(moduleMatch?.module_code || '', 'gi'), '')
                        .trim() || line;
        ...
        if (created.length) await fetchTasks();
        setBulkImportText('');
        setBulkImportFeedback(
            [created.length ? `${created.length} added` : null, skipped.length ? `${skipped.length} skipped` : null]
                .filter(Boolean)
                .join('. ')
        );
                ...
                {modules.find((m) => m.id === task.module_id) && (
                    <div className="flex items-center gap-2">
                        <span
                            ...
                            style={{
                                backgroundColor: modules.find((m) => m.id === task.module_id)?.color || '#e2e8f0'
                            }}
                        />
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-emerald-50">
            ...
            <div className="container mx-auto px-4 py-8">
                ...
                {error && (
                    <p className="mb-4 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-red-700">{error}</p>
                )}

                {/* Guided Creator */}
                <form
                    onSubmit={handleGuidedTaskSubmit}
                    className="mb-8 overflow-hidden rounded-3xl bg-gradient-to-br from-indigo-600 via-purple-500 to-pink-500 p-8 shadow-2xl"
                >
                    <div className="mb-6 space-y-2">
                        ...
                        <p className="text-purple-100">
                            Create tasks quickly by selecting module, task type, urgency, and deadline.
                        </p>
                    </div>
                    ...
                    {!showGuidedForm ? (
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid gap-4 sm:grid-cols-2">
                                <div>
                                    ...
                                    <select
                                        ...
                                        className="w-full rounded-xl border-0 bg-white/90 px-4 py-2 text-slate-900 shadow-md"
                                    >
                                ...
                                <div>
                                    <label className="block text-sm font-semibold text-white mb-2">Task Type</label>

                                    <select
                                        value={guidedTaskForm.taskType}
                                        onChange={(e) =>
                                            setGuidedTaskForm((s) => ({ ...s, taskType: e.target.value }))
                                        }
                                        className="w-full rounded-xl border-0 bg-white/90 px-4 py-2 text-slate-900 shadow-md"
                                    >
                                ...
                                {/* urgency + deadline */}
                                <div>
                                    <label className="block text-sm font-semibold text-white mb-2">Urgency *</label>
                            ...
                            <div>
                                <label className="block text-sm font-semibold text-white mb-2">Task Title (Optional)</label>
                                <input
                                    type="text"
                                    value={guidedTaskForm.title}
                                    onChange={(e) => setGuidedTaskForm((s) => ({ ...s, title: e.target.value }))}
                                    placeholder={`e.g., ${generateTaskTitle(guidedTaskForm)}`}
                                    className="w-full rounded-xl border-0 bg-white/90 px-4 py-2 text-slate-900 shadow-md"
                                />
                                <p className="mt-1 text-xs text-purple-100">
                                    Leave empty to auto-generate: {generateTaskTitle(guidedTaskForm)}
                                </p>
                            </div>
                            ...
                            <div className="flex gap-3 justify-end pt-4">
                                <button
                                    ...
                                    className="rounded-xl bg-white/20 border border-white/30 px-6 py-2 font-semibold text-white"
                                >
                                    Cancel
                                </button>
                ...
                <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <PrimaryButton
                        ...
                        className="w-auto rounded-xl px-6 py-3 font-semibold"
                    >
                            ...
                            <div className="mb-6 flex items-center justify-between">
                                <div>
                                    ...
                                    <p className="mt-1 text-sm text-slate-600">
                                        Paste one task per line. If module code is present it will be used,
                                        otherwise first module is fallback.
                                    </p>
                                </div>

                                <button onClick={() => setShowBulkImportModal(false)} className="text-slate-400 text-xl">
                                    ✕
                                </button>
                            </div>
                        ...
                        {(() => {
                            ...
                            return columns.map((column) => {
                                ...
                                return (
                                    <div
                                        key={column.status}
                                        className={`rounded-2xl border-2 ${colorClasses[column.color]} overflow-hidden shadow-sm`}
                                    >
                                                ...
                                                <div className="rounded-2xl border-2 border-dashed border-slate-300 bg-white p-8 text-center">
                                                    ...
                                                    <p className="text-sm text-slate-500">
                                                        {column.status === 'pending'
                                                            ? 'No tasks waiting. Add one with guided creator.'
